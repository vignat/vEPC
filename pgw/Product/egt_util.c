...
/********************************************************************20**
     ...
     Sid:      egt_util.c@@/main/nodal_main/lp_crfix_branch/1 - Wed Feb 18 15:11:41 2015
...
#else
PUBLIC S16 egUtlEgtAddAmbr(egMsg, egLvlInfo, ambr, inst)
EgMsg            *egMsg;
EgIeLevelInfo    *egLvlInfo;
U8               *ambr;
U8               inst;
...
#ifndef EG_REL_930
   egUtlEgtBuildEgIeHeader(&egIe,EG_AMBR_IETYPE, inst, 0, EG_AMBR);
#else
   egUtlEgtBuildEgIeHeader(&egIe,EG_AMBR_IETYPE, inst, EG_AMBR);
#endif /* EG_REL_930 */

   cmMemcpy((U8 *)&egIe.t.ambr.uplAPNAMBR, ambr, 4);
   cmMemcpy((U8 *)&egIe.t.ambr.dnlAPNAMBR, ambr + 4, 4);
   if(EgUtilGmAppendIe(egMsg, egLvlInfo, &egIe) != ROK)
   {
      RETVALUE(RFAILED);
   }
...
#else
PUBLIC S16 egUtlEgtAddPCO(egMsg, egLvlInfo, pco, len, inst)
EgMsg            *egMsg;
EgIeLevelInfo    *egLvlInfo;
U8               *pco;
U8               len;
...
#ifndef EG_REL_930
   egUtlEgtBuildEgIeHeader(&egIe,EG_PCO_IETYPE, inst, 0, EG_STRING_MAX);
#else
   egUtlEgtBuildEgIeHeader(&egIe,EG_PCO_IETYPE, inst, EG_STRING_MAX);
#endif /* EG_REL_930 */

   egIe.t.valStr16.length = len;
   cmMemcpy( (U8 *)egIe.t.valStr.val, (U8 *)(pco) ,egIe.t.valStr.length);
...
#endif /* EG_REL_930 */
{
   ...
   if(ipAddr)
   {
      ...
      cmMemcpy((U8 *)egIe.t.fTeid.ip4Addr, (U8 *) ipAddr, 4 * sizeof(U8));
   }
   else
   {
...
/********************************************************************30**
         ...
         End of file:     egt_util.c@@/main/nodal_main/lp_crfix_branch/1 - Wed Feb 18 15:11:41 2015
