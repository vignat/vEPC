...
/********************************************************************20**
     ...
     Sid:      tl_bdy3.c@@/main/6 - Mon Mar  3 20:10:07 2008
...
#endif
{
   ...
   *qBuf = NULLP;
   ...
   TAKE_TIMESTAMP("L/T Before DatReq/cmInetSendMsg(), in HI");
      ...
      /*hi034.201: while re-transmtting unpack the sctp parameters and use the appropirate API*/
      {
...
/*hi014.105  1. Modified statistics req to include messages Tx/Rx in bytes per con per thread */
      ...
      /*hi034.201: packing the sctp parameters which will be required for re-transmission*/
      *qBuf = mBuf; 
      mBuf = NULLP;
   }
...
/*hi014.105  1. Modified statistics req to include messages Tx/Rx in bytes per con per thread */
      ...
      else if (conCb->flag & HI_FL_UDP)
      {
         ...
         if (usrSrvc  ||  conCb->srvcType == HI_SRVC_UDP_TPKT_HDR)
...
/* hi013.105 :  Added two new HIT primitives. HiUiHitTlsEstReq(), HiUiHitTlsEstCfm().*/
          ...
          */
         if (usrSrvc  ||  conCb->srvcType == HI_SRVC_TCP_TPKT_HDR)
         {
...
/********************************************************************30**
         ...
         End of file:     tl_bdy3.c@@/main/6 - Mon Mar  3 20:10:07 2008
...
/main/6+   hi034.201  mladdha 1. Added support for multiple post structures in
                                 SCT interface.
                              2. Made lock changes in hiProcessSctpNtfyComUp().
                              3. Sctp parameters which will be needed if re-tranmission
                                 was required are packed properly in hiSctpSndMsg()
                                 and hiTxMsg().
                              4. Added cmInetAbortSctpAssoc fucntion which sends ABORT 
                                 to peer when sctp_peeloff fails.
*********************************************************************91*/
