...
/********************************************************************20**
     ...
     Sid:      leg.c@@/main/13 - Mon Sep 22 17:18:27 2014
...
#ifdef EGTP_C
/* leg_c_001.main_12: ccpu00139510 Egtpc release 10 upgrade */
#ifdef EGTP_C_PTH_MGT
   CMCHKPK(SPkU8, genCfg->rstCnt, mBuf);
   CMCHKPK(cmPkBool, genCfg->prn, mBuf);
   CMCHKPK(cmPkBool, genCfg->mabr, mBuf);
   CMCHKPK(cmPkBool, genCfg->ntsr, mBuf);
...
#ifdef EGTP_C
   if (tsrvCfg->noOfAssocUSap)
   {
      for(idx=tsrvCfg->noOfAssocUSap-1; idx >=0; idx--)
      {
         CMCHKPK(cmPkSpId, tsrvCfg->uSapIds[idx], mBuf);
      }
   }
   CMCHKPK(SPkU16, tsrvCfg->noOfAssocUSap, mBuf);
   CMCHKPK(cmPkBool, tsrvCfg->isDfltSrv, mBuf);
#endif
...
/* leg_c_001.main_12: ccpu00139510 Egtpc release 10 upgrade */
   CMCHKUNPK(cmUnpkBool, &genCfg->ntsr, mBuf);
   CMCHKUNPK(cmUnpkBool, &genCfg->mabr, mBuf);
   CMCHKUNPK(cmUnpkBool, &genCfg->prn, mBuf);
...
#ifdef EGTP_C
   CMCHKUNPK(cmUnpkBool, &tsrvCfg->isDfltSrv, mBuf);
   CMCHKUNPK(SUnpkU16, &tsrvCfg->noOfAssocUSap, mBuf);
   if (tsrvCfg->noOfAssocUSap)
   {
      for(idx=0;idx< tsrvCfg->noOfAssocUSap; idx++)
      {
         CMCHKUNPK(cmUnpkSpId, &tsrvCfg->uSapIds[idx], mBuf);
      }
   }
#endif
...
/********************************************************************30**
         ...
         End of file:     leg.c@@/main/13 - Mon Sep 22 17:18:27 2014
...
/main/13     ---      leg_c_001.main_12   rss      1. Egtpc Release 10 upgrade
*********************************************************************91*/
