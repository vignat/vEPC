...
/********************************************************************20**
     ...
     Sid:      pg_li.c@@/main/3 - Fri Feb  4 18:18:15 2011
...
PRIVATE S16 egLiPrcBndCfm ARGS((
         SuId        suId,
         EgTSapCb    *tsapCb,
         U16         result));
...
#endif
{
   ...
   /* eg012.201 : removed for klocwork fix */
   if (!localAddr)
   {
      EG_DBG_ERR(0,0, (egp,"\n localAddr: NULLP "));
      /* klocwork fix */
...
#endif
{
   ...
   S16            retVal;     /* Return value */
   EgUsta         sta;        /* Unsolicited status diagnostics structure */ 
   /* Use local pst structure and modify it locally */
#ifdef EGTP_U
   Pst            tempPst;
#endif /* EGTP_U */
#ifdef TIME_MES
#ifdef SS_MT_CB
   if (entryRx[pst->dstInst-1].call < MAX_MSG)
   {
      clock_gettime(0, &entryRx[pst->dstInst-1].time[entryRx[pst->dstInst-1].call++]);
   }
#else
   PRIVATE U32 call;
   if (call < MAX_MSG)
   {
      clock_gettime(0, &entryRx1[call]);
      call ++;
   }
#endif
#endif
...
#ifndef SS_MT_CB
   /******************************************
    * Check any control operation is pending *
    ******************************************/
   if (tSapCb->pendOp == TRUE)
   {
      EG_DBG_ERR(0,0,(egp,"\n TSap Control opertion is in progress"));
   ...
   *************************************************/
   ...
   (Void) egTptGetMsg (
      ...
      EG_RDWR_UNLOCK(&egCb.egUCb.threadLock)
#endif /* SS_MT_CB */
/* eg004.201 Send run-time update when Data Indication received */
#ifdef HW
      HW_UPD_PEER();
#endif
   EG_RETVALUE(ROK);
...
#endif
{
   ...
   if (serverCb != NULLP)
   {
      ...
      if(!serverCb->cfg.gtp_C_Or_U)
{
      /* Delete the Context and Inform User*/
      (Void) egTptDelAllServerContext (serverCb, TRUE);
} /* End of if */
...
#else
PRIVATE S16 egLiPrcBndCfm(suId, tsapCb, result)
SuId        suId;
EgTSapCb    *tsapCb;
U16         result;
#endif
...
/********************************************************************30**
         ...
         End of file:     pg_li.c@@/main/3 - Fri Feb  4 18:18:15 2011
