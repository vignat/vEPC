...
/********************************************************************20**
     ...
     Sid:      tl_bdy2.c@@/main/6 - Mon Mar  3 20:10:03 2008
...
#endif
{
   ...
   /*hi034.201: The locking mechanism has been reverted back to 27th patch level*/
#ifdef HI_MULTI_THREADED
   if (ret == ROK)
      HI_LOCK(&((*conCb)->lock));
#endif
...
#endif
{
   ...
   *conCb = prevConCb = NULLP;
               ...
               (PTR *)conCb)) == ROK)
   {
      ...
      {
         /* hi034.201: reverted back the locking changes */
#ifdef HI_MULTI_THREADED
         HI_LOCK(&((*conCb)->lock));
#endif
...
#endif
{
   ...
   /* hi034.201: Removed hiFindConcb functionality */
#ifdef HI_MULTI_THREADED
   HI_LOCK(&sap->conCbHlLock);
#endif

   /* find the connection control block in the SAP hash list */
   ret = cmHashListFind(&sap->conCbHlCp, (U8 *)&tMsg->spConId, sizeof (UConnId),
         0, (PTR *)&conCb);
   ...
   if (ret != ROK)
   {
      ...
      /* hi034.201: releasing Sap lock in the failure case*/
#ifdef HI_MULTI_THREADED
      HI_UNLOCK(&sap->conCbHlLock);
#endif
      RETVOID;
...
/*hi014.105  1. Modified statistics req to include messages Tx/Rx in bytes per con per thread */
  ...
  /*hi028.201: Handled statistics properly*/
   ...
   /*  hi034.201: removed HI_FREECONCB and made changes in the definition*/
   sap = conCb->sap;
   cmHashListDelete(&sap->conCbHlCp, (PTR)conCb);
   ...
   if (conCb->icmpError)
      HI_FREE(conCb->numFilters * sizeof(CmIcmpError),
            conCb->icmpError);
/* hi035.201: added 'if' check for multi threaded case */   
#ifdef HI_MULTI_THREADED
   SDestroyLock(&conCb->lock);
   if(conCb->flag & HI_FL_TCP)
      SDestroyLock(&conCb->txQLock);
#endif   
   HI_FREE(sizeof(HiConCb), conCb);
...
#endif
{
   ...
   switch(evnt)
   {
      case HI_TMR_SCHD:
      {
         ...
         /* hi034.201: added 'if' check to start the timer*/
         if(hiCb.init.cfgDone)
         {
            HI_START_TMR(HI_TMR_SCHD, 1);
         }
...
/********************************************************************30**
         ...
         End of file:     tl_bdy2.c@@/main/6 - Mon Mar  3 20:10:03 2008
...
/main/6+    hi010.201  rss   1. since windows doesn't has icmpv6 filter so the icmpv6 filter 
										  feature is supported under the flag ICMPV6_FILTER_SUPPORTED.
										  User has to enable the above flag to get the feature
$SID$       hi015.201  rcs   1. Moved the HI_CLOSE_SOCKETi macro
                                outside the HI_MULTI_THREADED flag in
                                hiDeInitFdGrp function.
...
/main/6+   hi032.201  ragrawal 1. Removed locking and unloacking of lmpst lock
                               ...
                               3. Added support for filteration of ICMP messages 
                                  ...
                                  under CM_ICMP_FILTER_SUPPORT flag.
/main/6+   hi034.201  mladdha 1. Added support for multiple post structures in
                                 SCT interface.
                              2. Made changes related to Locking mechanism in
                                 hiFindConcb() by reverting to 27th patch level.
                              3. Changed locking mechanism in hiFindConCbSuConId()
                              4. Start the timer for HI_TMR_SCHD only if
                                 init.cfgDone was TRUE.
/main/6+   hi035.201  mladdha 1. Added 'if' check for multi-threaded case                                 
*********************************************************************91*/
