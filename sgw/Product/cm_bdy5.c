...
/********************************************************************20**
     ...
     Sid:      cm_bdy5.c@@/main/nodal_main/1 - Thu Nov 21 09:40:20 2013
...
#ifdef MSPD_MLOG_NEW
   volatile U32 t1 = GetTIMETICK();   
#endif
#ifdef MSPD_DBG_ENABLE
...
#ifdef CMDBG
{
...
#ifdef MSPD_DBG_ENABLE
    /*  MSPD_DBG("Removed from Core:%d, Queue %d %x tmp1->tqExpire %d tmrEvnt %d\n", coreId, entry, tmp1,
                         tmp1->tqExpire, tmp1->tmrEvnt); */                       
#endif 
            tmp1->tmrEvnt = TMR_NONE; 
            tmp1->tqExpire = 0;     
            tmp1->cb = NULLP;      
   ...
   /* Logging time taken by this task */
    MLogTask(PID_CM_PRC_TMR, RESOURCE_LARM, t1, GetTIMETICK());
#endif
   RETVOID;
} /* end of cmPrcTmr */
#endif /* SS_FAP */
...
/*added FAP modifications*/
   ...
   for (tmrNum = 0; tmrNum < arg->max; tmrNum++)
   {
      if (arg->timers[tmrNum].tmrEvnt == TMR_NONE)
      {
         ...
         target->entIdx   = ent;
         #ifdef MSPD_DBG_ENABLE
         /* MSPD_DBG("Added Queue %d %x tmp1->tqExpire %d tmrEvnt %d tmrNum %d core %d arg->max %d\n",
		 	           ent, target,target->tqExpire, target->tmrEvnt, tmrNum, coreId, arg->max); 
				   */
         #endif
...
/* cm_bdy5_c_002.113 - Modification for SRegCfgTmr support */
   ...
   if (target->tmrEvnt != TMR_NONE)
   {
      ...
      while ((tmp1 = *tmp2) != NULLP)
      {
         if (tmp1 == target)
         {
            ...
            #ifdef MSPD_DBG_ENABLE
	    /*
			    MSPD_DBG("Stop from Queue %d tmp1->tqExpire %d tmrEvnt %d\n",tqEnt1,
                         tmp1->tqExpire, tmp1->tmrEvnt);			
			 */
            #endif
            break;
   ...
   if (target->tmrEvnt != TMR_NONE)
   {
      ...
      #ifdef MSPD_DBG_ENABLE
      /*
      MSPD_DBG("Removed from Queue %d %x tmp1->tqExpire %d tmrEvnt %d\n",ent, target,
                         target->tqExpire, target->tmrEvnt); 
			 */
      #endif
...
/********************************************************************30**
         ...
         End of file:     cm_bdy5.c@@/main/nodal_main/1 - Thu Nov 21 09:40:20 2013
