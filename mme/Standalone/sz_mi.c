...
/********************************************************************20**
     ...
     Sid:      sq_mi.c@@/main/nodal_main/3 - Fri Nov 22 18:01:33 2013
...
#endif /* ASNI */
{
   ...
   {
#if (ERRCLASS & ERRCLS_DEBUG)
      SZLOGERROR(ERRCLS_DEBUG,ESZ075,0,
   ...
   switch (cfg->hdr.elmId.elmnt)
   {
      ...
      {
#if (ERRCLASS & ERRCLS_DEBUG)
         SZLOGERROR(ERRCLS_DEBUG, ESZ076, (ErrVal) cfg->hdr.elmId.elmnt,
...
#endif /* ASNI */
{
   ...
   {
#if (ERRCLASS & ERRCLS_DEBUG)
      SZLOGERROR(ERRCLS_DEBUG,ESZ078,0,
   ...
   switch(cntrl->hdr.elmId.elmnt)
   {
      ...
      default:
      {
         /* invalid element */
#if (ERRCLASS & ERRCLS_DEBUG)
         SZLOGERROR(ERRCLS_DEBUG, ESZ079, (ErrVal) cntrl->hdr.elmId.elmnt,
...
#endif /* ASNI */
{
   ...
   {
#if (ERRCLASS & ERRCLS_DEBUG)
      SZLOGERROR(ERRCLS_DEBUG,ESZ081,0,
   ...
   switch (sts->hdr.elmId.elmnt)
   {
      ...
      case STSZPEER:            /* Peer node statistics */
      {
         ...
         if (ret == RFAILED)
         {
            SZLOGERROR(ERRCLS_INT_PAR, ESZ082, ERRZERO,
      ...
      {
#if (ERRCLASS & ERRCLS_DEBUG)
         SZLOGERROR(ERRCLS_DEBUG, ESZ084, (ErrVal) sts->hdr.elmId.elmnt,
...
#endif /* ASNI */
{
   ...
   {
#if (ERRCLASS & ERRCLS_DEBUG)
      SZLOGERROR(ERRCLS_DEBUG,ESZ086,0,
   ...
   switch (sta->hdr.elmId.elmnt)
   {
      ...
      {
#if (ERRCLASS & ERRCLS_DEBUG)
         SZLOGERROR(ERRCLS_DEBUG, ESZ087, (ErrVal) sta->hdr.elmId.elmnt,
...
#endif /* ASNI */
{
   ...
   {
      SZLOGERROR(ERRCLS_DEBUG, ESZ088, (ErrVal)0,
...
#endif /* ASNI */
{
   ...
   if (mBuf != NULLP)
   {
                  ...
                  szCb.init.lmPst.pool, &trcBuf)) != ROK)
      {
         ...
         SZLOGERROR(ERRCLS_ADD_RES, ESZ090, (ErrVal)0, "SCpyMsgMsg() failed.");
#endif /* (ERRCLASS & ERRCLS_ADD_RES) */
         SZ_FREE_BUF(trcBuf);
         RETVALUE(RFAILED);
      }
      ...
      if (trcLen > 0  &&  trcLen < ml)
      {
         ...
         {
#if (ERRCLASS & ERRCLS_ADD_RES)
            SZLOGERROR(ERRCLS_ADD_RES, ESZ091, (ErrVal)0, "SSegMsg() failed.");
#endif /* (ERRCLASS & ERRCLS_ADD_RES) */
            SZ_FREE_BUF(trcBuf);
            SZ_FREE_BUF(tmpBuf);
            RETVALUE(RFAILED);
...
#endif /* ASNI */
{
   ...
   {
#ifdef SZ_MME
      szCb.genCfg.dynamPeerFlag = gen->dynamPeerFlag;
      szCb.genCfg.defDPeerUSapId = gen->defDPeerUSapId;
#endif /* SZ_MME */
...
#ifdef SZ_MME
   if ((gen->dynamPeerFlag != TRUE) && (gen->dynamPeerFlag != FALSE))
   {
      RETVALUE(LCM_REASON_INVALID_PAR_VAL);
   }
#endif /* SZ_MME */
...
#ifdef SZ_MME
      case LSZ_NODE_MME:
         cmMemcpy(szCb.nodeDbg, (CONSTANT U8*)"S1AP: MME", SZ_DBGSIZE);
         break;
#endif /* SZ_MME */
#ifdef LSZV1
      case LSZ_NODE_GW:
...
#endif /* ANSI */
{
   ...
   for (loopCnt = 0; loopCnt < peer->nmbPeers;loopCnt++)
   {
      ...
      if (peerCb != NULLP)
      {
         ...
         {
#if (ERRCLASS & ERRCLS_INT_PAR)
            SZLOGERROR(ERRCLS_INT_PAR, ESZ092, (ErrVal)peerCb->peerId,
      ...
      {
#if (ERRCLASS & ERRCLS_ADD_RES)
         SZLOGERROR(ERRCLS_DEBUG, ESZ093, ERRZERO,
      ...
      case AUBND_DIS:
         {
            if (cntrl->u.sap.id < szCb.genCfg.nmbSztSaps)
            {
               ...
               if (sztSapCb != NULLP)
               {
                  ret = szUbndSztSap (cntrl->u.sap.id, SZ_UBND_SAP);
#if (ERRCLASS & ERRCLS_ADD_RES)
                  if (ret != ROK)
                  {
                     SZLOGERROR(ERRCLS_DEBUG, ESZ095, ERRZERO,
                           "szSztSapCntrl, Failed to unbind and disable \
                           the SZT SAP");
                  }
#endif /* (ERRCLASS & ERRCLS_ADD_RES) */
               }
      ...
      case ADEL:
         {
            if (cntrl->u.sap.id < szCb.genCfg.nmbSztSaps)
            {
               ...
               if (sztSapCb != NULLP)
               {
                  ret = szDelSztSap (cntrl->u.sap.id,SZ_DEL_SAP);
#if (ERRCLASS & ERRCLS_ADD_RES)
                  if (ret != ROK)
                  {
...
#endif /* ANSI */
{
   ...
   switch (cntrl->action)
   {
      ...
      case ABND_ENA:
      {
         ret = szEnaSctSap (cntrl->u.sap.id);
#if (ERRCLASS & ERRCLS_ADD_RES)
         if (ret != ROK)
         {
      ...
      case AUBND_DIS:
      {
         ret = szUbndSctSap (cntrl->u.sap.id,SZ_UBND_SAP, ENTSM);
#if (ERRCLASS & ERRCLS_ADD_RES)
         if (ret != ROK)
         {
      ...
      case ADEL:
      {
         ret = szDelSctSap (cntrl->u.sap.id, SZ_DEL_SAP);
#if (ERRCLASS & ERRCLS_ADD_RES)
         if (ret != ROK)
         {
      ...
      case ADEL:
         {
            ...
            for (loopCnt = 0; loopCnt < cntrl->u.peer.nmbPeers; loopCnt++) 
            {
               ...
               {
#if (ERRCLASS & ERRCLS_ADD_RES)
                  SZLOGERROR(ERRCLS_DEBUG, ESZ100,  cntrl->u.peer.peerId[loopCnt].val,
                        "szPeerCntrl, Peer not present");
#endif /* (ERRCLASS & ERRCLS_ADD_RES) */
                  continue;
               }

               if ((ret = szDelPeer(peerCb)) != ROK)
               {
#if (ERRCLASS & ERRCLS_ADD_RES)
                  SZLOGERROR(ERRCLS_DEBUG, ESZ101, ERRZERO,
...
#endif /* ANSI */
{
   ...
   ret = szNdbChkPeer(sts->peerId, &peerCb);
#if (ERRCLASS & ERRCLS_ADD_RES)
   if (ret != ROK)
   {
...
#endif /* ANSI */
{
   ...
   {
#if (ERRCLASS & ERRCLS_DEBUG)
      SZLOGERROR(ERRCLS_DEBUG,ESZ104,0,"Invalid parameter value");
#endif /* (ERRCLASS & ERRCLS_DEBUG) */
      ...
      if (szCb.sctSapLst[i] != (SzSctSapCb *) NULLP)
      {
               ...
               (sctSapCb->contEnt != ENTSM))
         {
            ...
            if (reqInfo->reqType == SHT_REQTYPE_BND_ENA)
            {
               ...
               {
#if (ERRCLASS & ERRCLS_DEBUG)
                  SZLOGERROR(ERRCLS_DEBUG,ESZ105,0,
            ...
            else
            {
               ...
               {
#if (ERRCLASS & ERRCLS_DEBUG)
                  SZLOGERROR(ERRCLS_DEBUG,ESZ106,0,"SzMiShtCntrlReq,\
   ...
   if( reqInfo->reqType == SHT_REQTYPE_UBND_DIS )
   {
      ...
      for (i = 0; i < szCb.genCfg.nmbSztSaps; i++)
      {
               ...
               (sztSapCb->pst.dstInst == reqInfo->s.ubndDis.dstEnt.inst))
         {
            ...
            {
#if (ERRCLASS & ERRCLS_DEBUG)
               SZLOGERROR(ERRCLS_DEBUG,ESZ107,0,
...
#endif /* ANSI */
{
...
#ifdef SS_MULTIPLE_PROCS
   ret = SGetXxCb(pst->dstProcId, pst->dstEnt,
                  pst->dstInst, (Void **)&szCbPtr);
#if (ERRCLASS & ERRCLS_DEBUG)
   if(ret !=ROK || (szCbPtr ==  NULLP))
   {
      SZLOGERROR(ERRCLS_DEBUG,ESZ108,0,
   ...
   {
#if (ERRCLASS & ERRCLS_DEBUG)
      SZLOGERROR(ERRCLS_DEBUG,ESZ109,0,
...
/**********************************************************************
         ...
         End of file:     sq_mi.c@@/main/nodal_main/3 - Fri Nov 22 18:01:33 2013
