...
/********************************************************************20**
     ...
     Sid:      sb_bdy3.c@@/main/3 - Thu Jul 26 07:55:03 2007
...
/* sb030.103:  Added to support 64 Bit compilation. */
   ...
   else
   {
      ...
      assocCb->sbAcCb.rwnd = GET_SB_MIN((aRwnd - assocCb->sbAcCb.bytesOut), assocCb->sbAcCb.rwnd);
   }
...
/* Sb035.103: Fixed the warnings when LONG_MSG is enabled.*/
   ...
   if ( N != 0 )
   {
      ...
      while ( stop == FALSE )
      {
          ...
          * the maximum pkArray size which is 200 in this case */
         if(idx >= PKARRAY_SIZE)
         {
...
/* sb030.103:  Added to support 64 Bit compilation. */
 ...
 /* sb030.103:  Added to support 64 Bit compilation. */
   ...
   {
      SBDBGP(SB_DBGMASK_AS, (sbGlobalCb.sbInit.prntBuf,
   "sbAsBuildInitAck: Hst Name Address (%s) is added in InitAck\n",
   ...
   {
       SBDBGP(SB_DBGMASK_AS, (sbGlobalCb.sbInit.prntBuf,\
       "sbAsBuildInitAck: Local Hst Name Address (%s) is added in InitAck\n",\
...
/* sb030.103:  Added to support 64 Bit compilation. */
   ...
   /* sb001.103: 9th SCTP Interop: Avoiding the insertion of Address parameters when only one interface is present */
   if ( tcb->tcbInitAck.localAddrLst.nmb != 1 )
   {
         ...
         case SB_CHUNK_ERROR_UNRSLV_ADDR  :
            ...
            if(errLen != SB_CHUNK_HEADER_SIZE)
            {
               ...
               else
               {
                  ...
                  SBDBGP(SB_DBGMASK_AS, (sbGlobalCb.sbInit.prntBuf,
                          "sbAsRcvError :  unresolved Address (%s)  \n", hstName));
               }
            }
            SB_ZERO(&rtrv, sizeof(SctRtrvInfo));
            /* in this state the whole assoc setup procedure simply stops */
     ...
     case SB_CHUNK_ERROR_UNRSLV_ADDR :    /* Unresolved Address */
            ...
            if(errLen != SB_CHUNK_HEADER_SIZE)
            {
               ...
               else
               {
                  ...
                  SBDBGP(SB_DBGMASK_AS, (sbGlobalCb.sbInit.prntBuf,
                           "sbAsRcvAbort: ABORT with unresolved Address (%s)  \n", hstName));
               }
            }
...
/********************************************************************30**
     ...
     End of file:     sb_bdy3.c@@/main/3 - Thu Jul 26 07:55:03 2007
