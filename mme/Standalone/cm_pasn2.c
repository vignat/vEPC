...
/********************************************************************20**
     ...
     Sid:      cm_pasn2.c@@/main/nodal_main/uchakala_feat_lpdev_sgsn_branch/1 - Fri Jul  4 12:56:36 2014
...
#endif
{
  ...
  if( specDef.ub <= ubMax ) 
  {
     ...
     for( i=0; i< len; i++)
     {
        ...
        {
           if(cmPAsnEncBitField(msgCp, (U8)v, b) != ROK) 
              RETVALUE(RFAILED);
        }
        else
        {
           /* b is more than 8 */
           if(cmPAsnEncByteField(msgCp,  v, b) != ROK)
               RETVALUE(RFAILED);
        }
        ...
        for( i =0; i< len; i++)
        {
           ...
           for( j=0; j< count; j++)
           {
              if( v == (U32)effAlp[j+1])
              {
                 ...
                 if( b <= CM_PASN_VAL_EIGHT)
                 {
                    if(cmPAsnEncBitField(msgCp,(U8)j, b) != ROK)
                       RETVALUE(RFAILED);
                 }
                 else
                 {
                    if(cmPAsnEncByteField(msgCp, (U32)j, b) != ROK)
                       RETVALUE(RFAILED);
                 }
  ...
  TRC2(cmPAsnEncByteField)
   ...
   if( b <= CM_PASN_VAL_EIGHT) 
   {
      if(cmPAsnEncBitField(msgCp, 
                 (U8)(GetLoByte(GetLoWord(val))), b) != ROK)
          RETVALUE(RFAILED);
   }
   else
   {
     ...
     if (msgCp->perType == CM_PASN_USE_UALIGN)
     {
        if(msgCp->bhldr.nxtBitPos != CM_PASN_VAL_EIGHT)
        {
           ...
           if(b <= pos)
           {
              if(cmPAsnEncBitField(msgCp,(U8)val,b) != ROK)
                 RETVALUE(RFAILED);
              RETVALUE(ROK);
           }
           else
...
#endif
{
   ...
   padBits  = (nBits - numBits) % CM_PASN_VAL_EIGHT;
	...
	{
		if (cmPAsnEncBitField(msgCp, val[0], nBits) != ROK)
			RETVALUE(RFAILED);
	}
   else
   {
      ...
      {
         if (cmPAsnEncMinOct(msgCp, byte, CM_PASN_ONE_OCTET) != ROK)
            RETVALUE(RFAILED);
      }

      if (padBits > 0)
      {
         if (cmPAsnEncBitField(msgCp, byte, padBits) != ROK)
            RETVALUE(RFAILED);
      }

      byte = val[0];

      if (cmPAsnEncBitField(msgCp, byte, msbBits) != ROK)
         RETVALUE(RFAILED);

      for (idx = 1; idx < valLen; idx++)
      {
         if (cmPAsnEncBitField(msgCp, val[idx], CM_PASN_VAL_EIGHT) != ROK)
            RETVALUE(RFAILED);
      }
   }
...
#endif
{
  ...
  if( (aub != alb) || ( aub >= CM_PASN_VAL_64K) )
  {
     if(aub < CM_PASN_VAL_64K)
     {
        ...
        if(cmPAsnEncConInt(msgCp, (U32)len, alb, aub, FALSE) != ROK)
           RETVALUE(RFAILED);
     ...
     else
     {
        if (len <= CM_PASN_VAL_16K)
        {
           ...
           /* fragmentation */
           if(cmPAsnEncUnconLen(msgCp, len) != ROK)
              RETVALUE(RFAILED); 
...
#endif
{
 ...
 if( !found)
 {
    ...
    if(cmPAsnEncBitField(msgCp,CM_PASN_VAL_ONE, CM_PASN_VAL_ONE) != ROK)
       RETVALUE(RFAILED);
 ...
 {
    /* Extension bit is 0 */
    if(cmPAsnEncBitField(msgCp, CM_PASN_VAL_NULL, CM_PASN_VAL_ONE) != ROK)
       RETVALUE(RFAILED);
 }
 RETVALUE(ROK);
...
#endif
{
  ...
  switch( lenVal)
  {
     case CM_PASN_ONE_OCTET:
        ...
        {
           if(cmPAsnEncBitField(msgCp,
                    (U8)(GetLoByte(GetLoWord(value))),CM_PASN_VAL_EIGHT) != ROK)
              RETVALUE(RFAILED);
        }
        else
        {
     ...
     case CM_PASN_TWO_OCTETS:
        ...
        {
           if(cmPAsnEncBitField(msgCp,
                    (U8)(GetHiByte(GetLoWord(value))),CM_PASN_VAL_EIGHT) != ROK)
              RETVALUE(RFAILED);
     ...
     case CM_PASN_THREE_OCTETS:
        ...
        {
           if(cmPAsnEncBitField(msgCp, 
                    (U8)(GetLoByte(GetHiWord(value))),CM_PASN_VAL_EIGHT) != ROK)
              RETVALUE(RFAILED);
           if(cmPAsnEncBitField(msgCp,
                    (U8)(GetHiByte(GetLoWord(value))),CM_PASN_VAL_EIGHT) != ROK)
              RETVALUE(RFAILED);
     ...
     case CM_PASN_FOUR_OCTETS:
        ...
        {
           if(cmPAsnEncBitField(msgCp,
                       (U8)(GetHiByte(GetHiWord(value))),CM_PASN_VAL_EIGHT) != ROK)
              RETVALUE(RFAILED);
           if(cmPAsnEncBitField(msgCp,
                       (U8)(GetLoByte(GetHiWord(value))),CM_PASN_VAL_EIGHT) != ROK)
              RETVALUE(RFAILED);
     ...
     case CM_PASN_XL_OCTETS:
       {
          ...
          if (msgCp->perType == CM_PASN_USE_UALIGN)
          {
             ...
             {
                if (cmPAsnEncBitField(msgCp, bytes[idx], 
                                             CM_PASN_VAL_EIGHT) != ROK)
                   RETVALUE(RFAILED);
             }
          }
          else
...
#endif
{
   ...
   if (msgCp->perType == CM_PASN_USE_UALIGN)
   {
      ...
      if(cmPAsnEncByteField(msgCp,val4bytes,N) != ROK)
         RETVALUE( RFAILED);
   ...
   else
   {
      ...
      {
         if(cmPAsnEncBitField(msgCp,(U8)val4bytes, cmPAsnGetBFSize(range)) != ROK)
            RETVALUE(RFAILED);
      ...
      if( range == CM_PASN_RANGE_256)
      {
         ...
         if(cmPAsnEncMinOct(msgCp, val4bytes, 1) != ROK)
            RETVALUE(RFAILED); 
            ...
            (range <= CM_PASN_RANGE_64K) )
      {
         ...
         if(cmPAsnEncMinOct(msgCp, val4bytes, CM_PASN_TWO_OCTETS) != ROK)
            RETVALUE(RFAILED); 
      ...
      if(cmPAsnEncConInt(msgCp, (U32)lenVal, CM_PASN_VAL_ONE, 
                  (U32)lenUb, FALSE) != ROK)
         RETVALUE(RFAILED);
      ...
      if(cmPAsnEncMinOct(msgCp, val4bytes, lenVal) != ROK)
         RETVALUE(RFAILED);
...
#endif
{
   ...
   if (msgCp->perType == CM_PASN_USE_UALIGN)
   {
      ...
      if (cmPAsnEncByteFieldXL(msgCp, valToEnc, valLen, numBits) != ROK)
         RETVALUE( RFAILED);
   ...
   else
   {
      ...
      if (rangeLen == 1)
      {
         if (cmPAsnEncBitField(msgCp,(U8)valToEnc[0],
                                      (U32)cmPAsnGetBFSize(range[0])) != ROK)
            RETVALUE(RFAILED);
      ...
      if ((rangeLen == 2) && (range[0] == 0x01) && (range[1] == 0x00))
      {
         ...
         /* Encode least significant byte of val4bytes */
         if (cmPAsnEncMinOct(msgCp, (U32)valToEnc[0], 1) != ROK)
            RETVALUE(RFAILED); 
      ...
      if ((rangeLen == 2) && !((range[0] == 0x01) && (range[1] == 0x00)))
      {
         ...
         /* Encode the value in 2 octets */
         if (cmPAsnEncMinOct(msgCp, (U32)*v, CM_PASN_TWO_OCTETS) != ROK)
            RETVALUE(RFAILED); 
      ...
      /* Encode "len" as Constrained Integer */
      if (cmPAsnEncConInt(msgCp, (U32)valLen, CM_PASN_VAL_ONE, 
                                 (U32)rangeLen, FALSE) != ROK)
         RETVALUE(RFAILED);
       ...
       * except when value to encode is zero
       */
      if (cmPAsnEncMinOct(msgCp, CM_PASN_VAL_NULL, CM_PASN_XL_OCTETS) != ROK)
         RETVALUE(RFAILED);
...
#endif
{
 ...
 if( lenVal < CM_PASN_UNCON_LEN2)
 {
    ...
    if(cmPAsnEncUnconLen(msgCp, lenVal) != ROK)
       RETVALUE(RFAILED);

    /* Encode the bytes containing the value  minimum octets */
    if(cmPAsnEncMinOct(msgCp, val4bytes, (U8)lenVal) != ROK)
       RETVALUE(RFAILED);
 } /* Non-fragmentation case */
 else
 {
    /* Fragmentation procedure of 10.9.3.8 to be applied */
    if(cmPAsnEncFragVal(msgCp, lenVal, 8) != ROK)
       RETVALUE(RFAILED);
 } 
 RETVALUE(ROK);
...
#endif
{
   ...
   {
      /* Encode "len" as an Unconstrained length determinant */
      if (cmPAsnEncUnconLen(msgCp, (U32)valLen) != ROK)
         RETVALUE(RFAILED);

      /* Encode the bytes containing the value minimum octets */
      for (idx = 0; idx < valLen; idx++)
      {
         if (cmPAsnEncMinOct(msgCp, valToEnc[idx], CM_PASN_ONE_OCTET) != ROK)
            RETVALUE(RFAILED);
      }
   }
   RETVALUE(ROK);
...
#endif
{
 ...
 if (msgCp->perType == CM_PASN_USE_UALIGN)
 {
    if(cmPAsnEncBitField(msgCp,length,CM_PASN_VAL_EIGHT) != ROK)
       RETVALUE(RFAILED);
 }
 else
 {
 ...
 if(cmPAsnEncMinOct(msgCp,(U32)value,length) != ROK)
    RETVALUE(RFAILED);
...
#endif
{
   ...
   if (msgCp->perType == CM_PASN_USE_ALIGN)
   {
	...
	if (msgCp->perType == CM_PASN_USE_UALIGN)
	{
		if (cmPAsnEncBitField(msgCp, len, CM_PASN_VAL_EIGHT) != ROK)
			RETVALUE(RFAILED);
	}
	else
	{
...
#endif
{
 ...
 if( lenVal <= CM_PASN_UNCON_LEN1)
 {
    ...
    if(cmPAsnEncMinOct(msgCp, lenVal, 1) != ROK)
       RETVALUE(RFAILED);
 }
          ...
          (lenVal < CM_PASN_UNCON_LEN2))
    {
       ...
       { 
          if (msgCp->perType == CM_PASN_USE_UALIGN)
          {
             ...
             if(cmPAsnEncBitField(msgCp,
                         (U8)GetHiByte(val2bytes),CM_PASN_VAL_EIGHT) != ROK)
                RETVALUE(RFAILED);
             /* Encode least significant byte  */
             if(cmPAsnEncBitField(msgCp,
                         (U8)GetLoByte(val2bytes),CM_PASN_VAL_EIGHT) != ROK)
                RETVALUE(RFAILED);
          }
          else
          {
...
#endif
{
  ...
  if( lenVal > CM_PASN_VAL_64K)
  {
     ...
     for( i = CM_PASN_VAL_ONE; i<=no64KComp; i++)
     {
       ...
       {
          if(cmPAsnEncBitField(msgCp,
                   (Data)CM_PASN_FRAG_CONST,CM_PASN_VAL_EIGHT) != ROK)
             RETVALUE(RFAILED);
       }
       else
       {
     ...
     if(m)
     {
        ...
        /* encode length first and then value items*/
         ...
         {
            if(cmPAsnEncBitField(msgCp,
                     (Data)(m | CM_PASN_FRAG_CONST1),CM_PASN_VAL_EIGHT) != ROK)
               RETVALUE(RFAILED);
         }
         else
         {
     ...
     if( noItems) 
     {
        if(cmPAsnEncUnconLen(msgCp, noItems) != ROK)
           RETVALUE(RFAILED);
     ...
     else
     {
        ...
        if (msgCp->perType == CM_PASN_USE_UALIGN)
        {
           if(cmPAsnEncBitField(msgCp,CM_PASN_VAL_NULL,
                       CM_PASN_VAL_EIGHT) != ROK)
              RETVALUE(RFAILED);
        }
        else
...
#endif
{
      ...
      (value <= CM_PASN_SMALLINT_VAL))
  {
     ...
     if(cmPAsnEncBitField(msgCp, (U8)value, (U8)CM_PASN_VAL_SIX) != ROK)
        RETVALUE(RFAILED);
  }
  else
  {
     ...
     if(cmPAsnEncSemiConInt(msgCp, value, CM_PASN_VAL_NULL, FALSE) != ROK)
        RETVALUE(RFAILED);
  } 
  RETVALUE(ROK);
...
#endif
{
      ...
      (lenValue <= CM_PASN_SMALLINT_LEN))
  {
     ...
     /* Encode value in 6 bit long bit field */
     if(cmPAsnEncBitField(msgCp, (U8)(lenValue-1), (U8)CM_PASN_VAL_SIX) != ROK)
        RETVALUE(RFAILED);
  }
  else
  {
     ...
     {
        if(cmPAsnEncUnconLen(msgCp, lenValue) != ROK)
           RETVALUE(RFAILED);
     }
               ...
               if( msgCp->evntStr->pres == PRSNT_NODEF)
               {
                  switch(type)
                  {
                     ...
                     case CM_PASN_TET_SINT8 :
                        /* MOD: Fixed warning for g++ compilation on Linux */
                        ...
                        /* MOD_END: Fixed warning for g++ compilation on Linux */
                        break;
                  ...
                  if(pres)
                  {
                     ...
                     if(cmPAsnEncBitField(msgCp, CM_PASN_VAL_ONE, 
                              CM_PASN_VAL_ONE) != ROK)
                        RETVALUE(RFAILED);
                  }
                  else
  ...
  *fragFlag = FALSE;
  ...
  if(elmDef->extMkr)
  {
         ...
         (numComp < min) )
     {
        ...
        {
           if(cmPAsnEncUnconLen(msgCp, numComp) != ROK)
              RETVALUE(RFAILED); 
        }
        else
...
/* cm_pasn2_c_002.main_13 */
      ...
      (elmSpecDef->max < CM_PASN_VAL_64K)) 
  {
     if(elmSpecDef->max != min)
     {
        ...
        if(cmPAsnEncConInt(msgCp, (U32)numComp ,
                 min, elmSpecDef->max, FALSE)!= ROK)
          RETVALUE(RFAILED);
     }
  }
  else
  ...
  *fragFlag = FALSE;
  ...
  if(elmDef->extMkr)
  {
         ...
         (length < min) )
     {
        ...
        {
           if(cmPAsnEncUnconLen(msgCp, (U32)length) != ROK)
              RETVALUE(RFAILED); 
        }
        else
           /* Fragmentation Required */
  ...
  if(max != min)
  {
     ...
     {
         /* Encode length as Constrained Integer */
         if(cmPAsnEncConInt(msgCp, (U32)length, min, max,FALSE)!= ROK)
             RETVALUE(RFAILED);
...
#endif
{
 ...
 if( (elmDef->type == CM_PASN_TET_OCTSTRXL) ||
     ...
     (elmDef->type == CM_PASN_TET_TPRNT_CONT) )
    isXlStr = TRUE;

  if(fragFlag)
  {
     /* If fragmentation is required */
     if(cmPAsnEncFragVal(msgCp, length, multiplier) != ROK)
        RETVALUE(RFAILED);
  ...
  if (msgCp->perType == CM_PASN_USE_UALIGN)
  {
      ...
      for( i =0; i < noBytes; i++)
      {
         ...
         if( isXlStr)
         {
            /* Extra Large String */
            if(cmPAsnEncBitField(msgCp,
                          (Data)(xlStr->val[i]),CM_PASN_VAL_EIGHT) != ROK)
               RETVALUE(RFAILED);
         }
         else
         { 
            if(cmPAsnEncBitField(msgCp, evntStr->val[i],
                     CM_PASN_VAL_EIGHT) != ROK)
               RETVALUE(RFAILED);
         }  
      }         
      if( strType == CM_PASN_BITSTR_TYPE)
      {
         ...
         if(cmPAsnEncBitField(msgCp, bitVal, noBits) != ROK)
               RETVALUE(RFAILED); 
      }
  }
  else
...
#endif
{
  ...
  if (SFndLenMsg(msgCp->mBuf, &msgLen) != ROK)
     RETVALUE(RFAILED);
  ...
  for(i=0; i< a; i++)
  {
    ...
    { 
       ...
       if(SCpyFixMsg((Data *)&encByte, msgCp->mBuf,
                           msgIdx, (MsgLen)CM_PASN_VAL_ONE, &cCnt) != ROK)
          RETVALUE(RFAILED);
       /* Increment the msgIdx by 64K octets */
  ...
  else
  {
           ...
           (m < CM_PASN_UNCON_LEN2))
     {
        ...
        /* Insert this segment length byte here */
        if(SCpyFixMsg((Data *)&pkArray[0], msgCp->mBuf,
                    msgIdx, (MsgLen)CM_PASN_VAL_TWO, &cCnt) != ROK)
           RETVALUE(RFAILED);
     }
  }
  RETVALUE(ROK);
...
#endif
{
  ...
  /* Now encode the length determinant for the left over components */ 
  if(cmPAsnEncUnconLen(msgCp, m) != ROK)
     RETVALUE(RFAILED); 
...
#endif
{
   ...
   if(lenBit)
   {
      U8 valBits = val[indx] << (CM_PASN_OSLEN_8 - lenBit);
...
/********************************************************************30**
         ...
         End of file:     cm_pasn2.c@@/main/nodal_main/uchakala_feat_lpdev_sgsn_branch/1 - Fri Jul  4 12:56:36 2014
...
/main/22     ---   psb    1. Fixed warning for g++ compilation on Linux
 *********************************************************************91*/
