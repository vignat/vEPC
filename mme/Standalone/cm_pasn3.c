...
/********************************************************************20**
     ...
     Sid:      cm_pasn3.c@@/main/nodal_main/lp_crfix_branch/1 - Mon Aug  4 11:28:59 2014
...
PRIVATE S16 cmPAsnDecTprntCont ARGS((CmPAsnMsgCp *msgCp)); 
...
CONSTANT CMPASNDECFUNC  cmPAsnDecFuncMtx[CM_PASN_MAX_TET] =
{
   ...
   cmPAsnDecTprntCont           /* 40 Transparent Container */
};
...
#ifdef ANSI
PRIVATE S16 cmPAsnDecTprntCont
(
CmPAsnMsgCp  *msgCp    /* message control pointer */
)
#else
PRIVATE S16 cmPAsnDecTprntCont(msgCp)
CmPAsnMsgCp  *msgCp;   /* message control pointer */
#endif
{
  Bool            fragFlag;   /* fragmntation Flag */
...
#endif
{
  ...
  /* cm_pasn3_c_001.main_9 */
  opcode = 0;
  fragFlag = FALSE; /* KLOCKWORK fix for UBUNTU */
  ...
  if( i == ieDef->classInf->maxInstances)
  {
     ...
     tBuf = msgCp->mBuf;
     ret = SSegMsg(tBuf, (MsgLen)len,&msgCp->mBuf);
     if(ret == RFAILED) 
         RETVALUE(RFAILED);
  ...
  /* Decode Unconstrained length determinant bytes */
      /* cm_pasn3_c_001.main_18 : kwork fix */
  if (msgCp->elmDef[0]->type != CM_PASN_TET_TPRNT_CONT)
  /*if (opcode != 61)*/
  {
      ret = cmPAsnDecUnconLen(msgCp, &len, &fragFlag);
      if(ret != ROK)
...
/********************************************************************30**
         ...
         End of file:     cm_pasn3.c@@/main/nodal_main/lp_crfix_branch/1 - Mon Aug  4 11:28:59 2014
